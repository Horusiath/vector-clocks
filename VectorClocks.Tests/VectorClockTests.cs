...
namespace VectorClocks.Tests
{
    public class VectorClockTests
    {
        ...
        public void A_VectorClock_must_pass_misc_comparison_test6()
        {
            ...
            clock3_1.IsConcurrentWith(clock3_2).Should().BeTrue();
            clock3_2.IsConcurrentWith(clock3_1).Should().BeTrue();
        }
        ...
        public void A_VectorClock_must_pass_misc_comparison_test7()
        {
            ...
            clock5_1.IsConcurrentWith(clock3_2).Should().BeTrue();
            clock3_2.IsConcurrentWith(clock5_1).Should().BeTrue();
        }
        ...
        public void A_VectorClock_must_pass_misc_comparison_test8()
        {
            ...
            clock4_1.IsConcurrentWith(clock1_2).Should().BeTrue();
            clock1_2.IsConcurrentWith(clock4_1).Should().BeTrue();
        }
        ...
        public void A_VectorClock_must_correctly_merge_two_clocks()
        {
            ...
            merged1.Count.Should().Be(3);
            merged1.ContainsNode(node1).Should().BeTrue();
            merged1.ContainsNode(node2).Should().BeTrue();
            merged1.ContainsNode(node3).Should().BeTrue();

            var merged2 = clock5_1.Merge(clock3_2);
            merged2.Count.Should().Be(3);
            merged2.ContainsNode(node1).Should().BeTrue();
            merged2.ContainsNode(node2).Should().BeTrue();
            merged2.ContainsNode(node3).Should().BeTrue();
        ...
        public void A_VectorClock_must_correctly_merge_two_disjoint_vector_clocks()
        {
            ...
            var merged1 = clock3_2.Merge(clock5_1);
            merged1.Count.Should().Be(4);
            merged1.ContainsNode(node1).Should().BeTrue();
            merged1.ContainsNode(node2).Should().BeTrue();
            merged1.ContainsNode(node3).Should().BeTrue();
            merged1.ContainsNode(node4).Should().BeTrue();

            var merged2 = clock5_1.Merge(clock3_2);
            merged2.Count.Should().Be(4);
            merged2.ContainsNode(node1).Should().BeTrue();
            merged2.ContainsNode(node2).Should().BeTrue();
            merged2.ContainsNode(node3).Should().BeTrue();
            merged2.ContainsNode(node4).Should().BeTrue();
        ...
        public void A_VectorClock_must_support_pruning()
        {
            ...
            c1.ContainsNode(node1).Should().BeFalse();
            c1.IsConcurrentWith(c).Should().BeTrue();

            (c.Prune(node1).Merge(c1)).ContainsNode(node1).Should().BeFalse();

            var c2 = c.Increment(node2);
            c1.IsConcurrentWith(c2).Should().BeTrue();
        }
        ...
        public void A_VectorClock_must_compare_as_Concurrent_when_Member_removed()
        {
            ...
            var a = VectorClock.Empty.Increment(node1).Increment(node2);
            var b = a.Prune(node2).Increment(node1); // remove node2, increment node1

            a.PartiallyCompareTo(b).Should().Be(null);
        }
    }
}